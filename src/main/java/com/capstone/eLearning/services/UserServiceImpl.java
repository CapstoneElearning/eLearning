...
public class UserServiceImpl implements UserService {
	...
	public int saveUser(User userDto) throws ServiceException {
		...
		mailService.sendMail(Constants.email, userDto.getEmail(),
				" eLearning - Activate Registration ", "Please click on the below link to activate your account ,Thanku. http://localhost:8080/eLearning/ActivationServlet?userId="+userDto.getEmail());
	...
	public boolean forgotpassword(String email) throws ServiceException {
		...
		if(flag==true){
		 mailService.sendMail(Constants.email, email, "Reset Password for eLearning", "Please click on the below link to Reset password for eLearning ,Thanku. http://localhost:8080/eLearning/ResetPasswordServlet?email="+email);
		}
		}catch(Exception e){
